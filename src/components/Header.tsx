...
export function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-outline-variant/80 bg-surface-container-lowest/72 px-4 py-3 backdrop-blur-xl sm:px-6">
        <div className="mx-auto w-full max-w-7xl md:hidden">
          <div className="flex min-w-0 items-center">
            <div className="inline-flex h-11 min-w-0 items-center gap-2 rounded-full border border-outline-variant/70 bg-surface-container-low px-3">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary text-on-primary">
                <span className="material-symbols-outlined text-[18px]">bolt</span>
              </span>
              <span className="truncate text-sm font-h3 leading-none text-on-surface">{dictionary.header.appName}</span>
            </div>

            <div className="ml-auto flex shrink-0 items-center gap-2">
              <button
                onClick={toggleTheme}
                className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-outline-variant/70 bg-surface-container-low text-on-surface-variant transition-colors hover:bg-surface-container"
              ...
              <button
                ...
                className="inline-flex h-11 min-w-[3.75rem] items-center justify-center gap-1 rounded-full border border-outline-variant/70 bg-surface-container-low px-2.5 text-on-surface-variant transition-colors hover:bg-surface-container"
        ...
        <div className="mx-auto hidden w-full max-w-7xl items-center justify-between gap-6 md:flex">
          <div className="flex min-w-0 items-center gap-8">
            <div className="inline-flex min-w-0 items-center gap-3 rounded-full border border-outline-variant/70 bg-surface-container-low px-3 py-1.5">
              ...
              <span className="truncate text-lg font-h3 text-on-surface">{dictionary.header.appName}</span>
            </div>
          ...
          <div className="flex shrink-0 items-center gap-4">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 font-button text-button text-on-primary shadow-lg shadow-primary/25 transition-transform duration-150 active:scale-95"
            >
              <span className="material-symbols-outlined text-[20px]">upload</span>
              <span>{dictionary.common.uploadFile}</span>
            </button>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <span className="text-xs font-button">{dictionary.header.theme}</span>
              </button>
              <button
                onClick={toggleLocale}
      ...
      <button
        onClick={() => setModalOpen(true)}
        className="soft-ring fixed right-4 bottom-4 z-[60] inline-flex h-14 w-14 items-center justify-center rounded-full bg-primary text-on-primary shadow-lg shadow-primary/30 transition-transform duration-150 active:scale-95 md:hidden"
        aria-label={dictionary.common.uploadFile}
        title={dictionary.common.uploadFile}
      >
